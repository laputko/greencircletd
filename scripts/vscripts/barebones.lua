...
-- GameRules Variables
 ...
 ROUND_UNITS = 3 -- кол-во юнитов на 1 раунде
...
function GameMode:OnGameInProgress() -- Функция начнет выполняться, когда начнется матч( на часах будет 00:00 ).
	print("[BAREBONES] The game has officially begun")
	  ...
	  local point1 = Entities:FindByName( nil, "spawnerino1"):GetAbsOrigin() -- Записываем в переменную 'point' координаты нашего спавнера 'spawnerino'
	  local point2 = Entities:FindByName( nil, "spawnerino2"):GetAbsOrigin() -- Записываем в переменную 'point' координаты нашего спавнера 'spawnerino'
	  local point3 = Entities:FindByName( nil, "spawnerino3"):GetAbsOrigin() -- Записываем в переменную 'point' координаты нашего спавнера 'spawnerino'
	  local point4 = Entities:FindByName( nil, "spawnerino4"):GetAbsOrigin() -- Записываем в переменную 'point' координаты нашего спавнера 'spawnerino'
      
      local waypoint1 = Entities:FindByName( nil, "way1.1") -- Записываем в переменную 'waypoint' координаты первого бокса way1.
      local waypoint2 = Entities:FindByName( nil, "way2.1") -- Записываем в переменную 'waypoint' координаты первого бокса way1.
      local waypoint3 = Entities:FindByName( nil, "way3.1") -- Записываем в переменную 'waypoint' координаты первого бокса way1.
      local waypoint4 = Entities:FindByName( nil, "way4.1") -- Записываем в переменную 'waypoint' координаты первого бокса way1.
      
      local return_time = 15 -- Записываем в переменную значение '15'
      Timers:CreateTimer(15, function()   -- Создаем таймер, который запустится через 15 секунд после начала матча и запустит следующую функцию. 
       GAME_ROUND = GAME_ROUND + 1 -- Значение GAME_ROUND увеличивается на 1.
         ...
         for i=1, ROUND_UNITS do -- Произведет нижние действия столько раз, сколько указано в ROUND_UNITS. То есть в нашем случае создаст 10 юнитов.
         	  count = count + 1.5
         	  Timers:CreateTimer(count, function()
					local unit = CreateUnitByName( "example_unit_" .. GAME_ROUND, point1 + RandomVector( RandomFloat( 0, 50 ) ), true, nil, nil, DOTA_TEAM_GOODGUYS ) -- Создаем юнита 'example_unit_', в конце к названию добавится 1,2,3,4 или 5, в зависимости от раунда, и в итоге получатся наши example_unit_1, example_unit_2 и т.д.. Юнит появится в векторе point + RandomVector( RandomFloat( 0, 200 ) ) - point - наша переменная, а рандомный вектор добавляется для того, чтобы мобы не появлялись в одной точке и не застревали. Мобы будут за силы света.
					unit:SetInitialGoalEntity( waypoint1 ) -- Посылаем мобов на наш way1, координаты которого мы записали в переменную 'waypoint'
					local unit = CreateUnitByName( "example_unit_" .. GAME_ROUND, point2 + RandomVector( RandomFloat( 0, 50 ) ), true, nil, nil, DOTA_TEAM_GOODGUYS ) -- Создаем юнита 'example_unit_', в конце к названию добавится 1,2,3,4 или 5, в зависимости от раунда, и в итоге получатся наши example_unit_1, example_unit_2 и т.д.. Юнит появится в векторе point + RandomVector( RandomFloat( 0, 200 ) ) - point - наша переменная, а рандомный вектор добавляется для того, чтобы мобы не появлялись в одной точке и не застревали. Мобы будут за силы света.
					unit:SetInitialGoalEntity( waypoint2 ) -- Посылаем мобов на наш way1, координаты которого мы записали в переменную 'waypoint'
					local unit = CreateUnitByName( "example_unit_" .. GAME_ROUND, point3 + RandomVector( RandomFloat( 0, 50 ) ), true, nil, nil, DOTA_TEAM_GOODGUYS ) -- Создаем юнита 'example_unit_', в конце к названию добавится 1,2,3,4 или 5, в зависимости от раунда, и в итоге получатся наши example_unit_1, example_unit_2 и т.д.. Юнит появится в векторе point + RandomVector( RandomFloat( 0, 200 ) ) - point - наша переменная, а рандомный вектор добавляется для того, чтобы мобы не появлялись в одной точке и не застревали. Мобы будут за силы света.
					unit:SetInitialGoalEntity( waypoint3 ) -- Посылаем мобов на наш way1, координаты которого мы записали в переменную 'waypoint'
					local unit = CreateUnitByName( "example_unit_" .. GAME_ROUND, point4 + RandomVector( RandomFloat( 0, 50 ) ), true, nil, nil, DOTA_TEAM_GOODGUYS ) -- Создаем юнита 'example_unit_', в конце к названию добавится 1,2,3,4 или 5, в зависимости от раунда, и в итоге получатся наши example_unit_1, example_unit_2 и т.д.. Юнит появится в векторе point + RandomVector( RandomFloat( 0, 200 ) ) - point - наша переменная, а рандомный вектор добавляется для того, чтобы мобы не появлялись в одной точке и не застревали. Мобы будут за силы света.
					unit:SetInitialGoalEntity( waypoint4 ) -- Посылаем мобов на наш way1, координаты которого мы записали в переменную 'waypoint'
              return nil
         	  end)   
         end  
      count = 0 
